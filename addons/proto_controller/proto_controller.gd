...
@export var swing_speed: float = 50.0
...
var current_hook_point: Hook_Point
...
func grap_swing(delta:float):
	...
	var direction = abs(transform.basis.z) * global_position.direction_to(current_hook_point.position)
	velocity += direction * swing_speed * delta
